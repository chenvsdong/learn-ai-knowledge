...
如果你第一次学习本章，先掌握最低闭环：定义输出契约，解析模型结果，做 Schema 校验，做引用和业务校验，再决定展示、重试、人工确认或拒绝执行。状态机、部署治理和技术栈映射是生产扩展，不要在还没跑通最小闭环前先追框架 API。

## 一个直观例子

主线交接：第 5 章已经把会议片段、上线检查清单和任务状态组织成上下文包；本章新增能力是让模型输出能被程序解析、校验、拒绝、修复或转人工。下一章会把外部知识库文档接入上下文，让答案绑定到可追踪来源。
...
- 每个行动项都有证据。
- 不确定性进入结构化字段。
- 模型只给出建议下一步，是否能写入任务系统仍由后端门禁计算。

再看第 5 章的知识库问答助手上线风险分析。这个例子只用于承接上一章，暂时只关注字段如何表达证据和下一步动作；工具调用细节会放到后续章节继续展开。用户问：
...
```text
主要风险是权限过滤和安全评审，建议继续跟进。
```

后端系统无法判断这个结果是否能进入风险看板、是否能触发下一步查询、是否需要人工确认。更好的输出是：
...
```json
{
  ...
  "evidence": [
    {
      ...
      "claim": "会议提到外部知识库访问前必须完成权限过滤验证"
    },
    {
      "type": "release_checklist",
      "source": "release-checklist-20260520",
      "claim": "上线检查清单要求安全评审完成后才能开放更大范围"
    }
  ],
  "unverified_items": [
    "尚未确认安全评审最终结论",
  ...
  "next_action": {
    ...
    "tool": "release_task.search",
    "reason": "确认安全评审和评估样本补充进度"
  },
  "needs_human_review": true
}
```

这个对象可以进入后端流程：如果 `confidence` 不足或 `unverified_items` 非空，就不自动生成确定结论；如果 `next_action.type` 是 `query_task_status`，系统可以判断工具是否允许、参数是否完整、是否需要审批。
...
| Tool Result Schema | 让工具返回值具备稳定结构 | 在本章引用的 MCP 2025-11-25 spec 中，工具可声明 `outputSchema`；其他平台通常需要应用侧自定义和校验 |

OpenAI 官方 Structured Outputs 支持通过 JSON Schema 约束模型输出；Function Calling 的 strict mode 也基于 structured outputs。需要注意，`additionalProperties: false`、属性需要标为 required 等是 OpenAI Structured Outputs 对其支持的 JSON Schema 子集提出的约束，不是 JSON Schema 规范本身的通用要求。Spring AI 文档也提醒，prompt-based structured output converter 是 best effort，模型不保证一定按要求输出，仍要做验证机制。
...
```json
{
  ...
  "unverified_items": ["security review result", "permission eval sample count"],
...
```json
{
  "type": "object",
  "properties": {
    "schema_version": {"type": "string", "enum": ["release-risk-v1"]},
    "analysis_status": {
      "type": "string",
      "enum": ["ready", "risks_found", "needs_more_evidence", "blocked"]
    },
    ...
    "risk_items": {
      ...
      "items": {
        ...
        "properties": {
          "risk_type": {
            "type": "string",
            "enum": ["permission", "safety_review", "evaluation", "rollback", "unknown"]
          },
          ...
          "evidence": {
            ...
            "items": {
              "type": "object",
              "properties": {
                "source_type": {"type": "string", "enum": ["meeting_note", "release_checklist", "risk_record", "task_status"]},
        ...
        "required": ["risk_type", "description", "evidence", "needs_human_review"],
        "additionalProperties": false
      }
    },
    "unverified_items": {
    ...
    "next_action": {
      "type": "object",
      "properties": {
        "type": {"type": "string", "enum": ["none", "ask_user", "query_task_status", "human_review"]},
...
这个 Schema 允许模型表达“不足以判断”。对于上线风险分析，这比强迫模型给确定结论更可靠。
...
OutputValidationResult validate(ModelOutput output, ContextPackage context) {
    ...
    SchemaResult schema = schemaValidator.validate(parsed.value(), "release-risk-v1");
    if (!schema.valid()) {
...
下面是结构化输出在常见技术栈中的落点速览，用来帮助读者建立映射关系。它不是 API 教程；本章主线仍然是“输出契约 -> 校验 -> 决策 -> 执行”。

| 技术栈 | 结构化输出落点 | 本章只需要记住的边界 |
| --- | --- | --- |
| Spring AI | `StructuredOutputConverter`、native structured output、DTO 反序列化 | Converter 成功只表示能转成对象，不代表能写入业务系统 |
| LangChain4j | AI Service 返回类型、JSON Schema、fallback 到 prompt-based 格式 | AI Service 不是确定性服务；发生 fallback 时要记录输出模式并加强校验 |
| OpenAI API | Structured Outputs、JSON Schema、Function Calling strict mode | 先处理拒绝、截断、不完整和供应商错误，再解析业务对象 |
| MCP | `inputSchema`、`outputSchema`、`structuredContent`、`isError` | 工具结果要结构化，但仍要权限、脱敏、大小限制和审计 |
| Claude Code | `--output-format json` / `stream-json`、hooks JSON、子 Agent 输出契约 | CLI 的 JSON 输出主要提供会话结果和元数据；任务级 JSON 契约仍应由 Prompt 约束、外部解析和 Schema 校验共同保证 |

跨技术栈的共同原则是：框架只能帮助你获得形状更稳定的输出，不能替你完成事实校验、引用校验、业务门禁、安全判断和副作用执行。模型生成 tool call 参数不代表工具必须执行；后端仍然要做权限、参数、预算、审批和幂等校验。
...
团队工具开始需要统一契约。一个上线风险分析助手输出的 `analysis_status`、`risk_items`、`evidence`、`next_action`，可能会被前端、风险看板、任务系统和评估系统同时使用。
...
建议按任务 1、3、4 的顺序完成；任务 2 连接第 5 章知识库问答助手上线风险分析，任务 5 是 Agent 工程扩展。
...
2. 初级：为上线风险分析助手设计 `OutputValidationResult`。
...
5. 可选扩展：为编码 Agent 审查输出设计结构化格式。

这不是本章主线任务，适合已经在做编码 Agent 自动化的读者。交付物：一个包含 `findings`、`severity`、`file`、`line`、`evidence`、`must_fix`、`summary` 的 JSON 契约。

自查标准：脚本能解析结果；空发现和高风险发现都有明确表示；输出不依赖自然语言关键词判断成功或失败。

参考答案要点：任务 1 的 Schema 应允许缺失负责人或截止日期用 `null` 表达，并用 `needs_human_review` 标记，不允许模型补猜。任务 2 的 `OutputValidationResult` 应能同时表达 parse 成功但 citation 失败、citation 成功但 business 失败、安全失败必须阻断。任务 4 的写入门禁至少要检查用户权限、字段完整性、证据引用、幂等键、人工确认状态和失败降级路径。
...
审稿指出工程实现部分信息密度较高，RAG 过渡偏概括，实践任务缺少难度分层，上线风险例子需要和第 5 章统一。正文已新增“最小闭环先掌握什么”过渡表、RAG 证据字段衔接说明、统一练习输入和入门到专业任务分层，并将第 5 章衔接例子统一为知识库问答助手上线风险分析。

### 主编

主编检查后保留会议纪要助手作为主线例子，并用知识库问答助手上线风险分析承接第 5 章；保留 OpenAI、Spring AI、LangChain4j、MCP、Claude Code 的框架映射，但强调具体 API 以官方文档为准。本章不展开 RAG 检索和 Agent 工具生命周期，只为第 7 章的证据引用、来源追踪和输出校验建立基础。
