...
截至 2026-05，OpenAI 的 Responses API 文档中可以通过 `instructions` 参数或消息角色表达应用开发者指令，并区分 developer、user、assistant 等角色；部分推理模型的最佳实践也强调 developer message 承担过去许多 system message 的职责。Chat Completions、Responses API、Spring AI、LangChain4j 和 MCP 的消息模型并不完全等价。本章用 System/User/Assistant 建立直觉，真正落地时要回到具体 API 和依赖版本。
...
### Claude Code：把 Prompt 变成编码 Agent 的项目记忆

Claude Code 这类编码 Agent 给 Prompt Engineering 提供了一个很好的生产化样本：Prompt 不再只是一段发给模型的临时文本，而会沉淀为项目记忆、规则、权限、工具和审查流程。

在 Claude Code 中，`CLAUDE.md` 是项目记忆的重要入口。官方文档建议把你会反复解释给 Claude 的信息写进去，例如构建命令、测试命令、代码风格、项目结构、常见工作流和“总是做 X”这类规则。它适合放稳定的团队共识，不适合放一次性任务细节、敏感密钥或过长流程。

如果一个仓库已经使用 `AGENTS.md` 给其他编码 Agent 提供规则，可以让 `CLAUDE.md` 导入 `AGENTS.md`，再补充 Claude Code 专属说明。对本书这个项目来说，`AGENTS.md` 里的章节写作协议就属于典型项目记忆：它不是某一轮对话的临时 Prompt，而是每次写章节都应该加载的工作契约。

这给 Agent 开发者一个非常实用的启发：长期规则不要全塞进用户每次输入里，而应该分层管理。
...
Claude Code 的另一个实践是权限和工具边界。它可以通过 settings 配置权限、环境变量和工具行为；还可以用 `permissions.deny` 阻止读取 `.env`、secrets、credentials 等敏感文件。这和本章反复强调的原则一致：不要靠 Prompt 告诉模型“不要读密钥”，而要让工具层和权限层真的拒绝访问。

Subagents 也体现了 Prompt 工程的分工思想。Claude Code 可以定义专门的子 Agent，每个子 Agent 有自己的系统提示、工具权限，甚至可以绑定特定 MCP server。技术审稿人、工程审稿人、学习体验审稿人这类角色，就很适合用子 Agent 隔离上下文和职责。这样做的价值不是“角色扮演更像样”，而是减少主作者视角造成的自我确认，让不同审查目标使用更干净的上下文。

Hooks 则说明 Prompt 之外还需要运行时控制。Claude Code hooks 可以在用户提交 Prompt、工具调用前后、权限请求、压缩上下文等生命周期节点运行。对生产 Agent 来说，这相当于把一部分安全、审计、格式检查和自动化流程放到运行时，而不是只写在 Prompt 里。例如，在高风险工具调用前做参数扫描，在写文件后自动运行格式检查，在用户提交 Prompt 时注入项目状态摘要。

这些能力共同说明一件事：编码 Agent 的可靠性不是靠一个巨大系统提示词堆出来的，而是靠“项目记忆 + 工具权限 + 生命周期钩子 + 子 Agent 分工 + 可观测调试”组成的工程系统。

如果 Claude Code 没有按预期遵循项目规则，官方文档也提供了很工程化的排查思路：用 `/context` 查看当前会话里到底加载了哪些系统提示、memory、skills、MCP tools 和对话消息；用 `/memory`、`/hooks`、`/mcp`、`/permissions` 等命令检查具体配置是否生效。这对所有 Agent 系统都有借鉴意义：当模型没有按预期工作时，先确认上下文是否真的进入模型、权限是否真的生效、工具是否真的可用，再去改 Prompt。

Claude Code 相关实践也有边界。`CLAUDE.md` 会占用上下文窗口，过长、冲突、含糊的规则会降低遵循质量；hooks 可以访问或修改用户权限下的文件，配置错误会造成真实副作用；子 Agent 的工具权限如果过宽，也会扩大风险面。因此，项目记忆要短而具体，权限要最小化，hooks 要可审计，子 Agent 要按职责授权。
...
OWASP LLM Top 10 把 Prompt Injection 和 Insecure Output Handling 列为重要风险。对 Agent 系统来说，Prompt Injection 的危险不只是回答变差，而是模型可能被诱导泄露数据、调用错误工具、生成危险参数或绕过业务流程。
...
日志方面，要记录足够复盘的信息，但不要无脑保存全文。可观测字段可以包括 prompt_template_id、prompt_version、model、task_type、risk_level、input_hash、variable_names、output_status、validation_error、latency、token_usage、fallback_action。审计字段可以包括 trace_id、request_id、tenant_id_hash、user_id_hash、resource_scope、data_classification、rendered_prompt_hash、output_hash、policy_decision、approval_id 和 retention_policy。原始输入和输出是否落库，要按数据敏感级别脱敏、采样、加密、限制访问并设置保留周期。
...
4. 编码 Agent 任务：为一个已有代码仓库设计一份最小 `CLAUDE.md` 或 `AGENTS.md`。内容只允许包含稳定项目事实：构建命令、测试命令、目录边界、代码风格、提交规则、安全禁止项。交付物是一份不超过 80 行的项目记忆文件，以及一张“哪些内容不应该写进去”的清单。验收标准是：新加入的工程师或编码 Agent 能用它完成一次小改动，但里面不包含密钥、个人偏好、一次性任务或过期命令。
...
- 修订动作：正文中明确“不同平台角色名不同，以官方文档和项目版本为准”；补充 OpenAI Responses API、developer message 和 `instructions` 的边界；将 OpenAI 部分写成工程边界而非 API 教程；补充模板语法不可复制；单独说明 MCP 的 `prompts/list`、`prompts/get`、Tools、Resources、Prompts 边界；新增 Claude Code 的 `CLAUDE.md`、settings、hooks、subagents、MCP 和 `/context` 调试边界。
- 结论：本章没有编造 API、版本或性能数字；涉及官方协议和框架的内容已按 2026-05 的官方文档核查，细节实现留到对应框架实战章节。

### 工程审稿人

- 发现问题：初稿如果只讲 Prompt 写法，容易停留在 Demo；需要补充模板仓库、变量校验、版本、灰度、日志、失败回放、权限边界、异常矩阵、多租户过滤、发布门禁，以及编码 Agent 的项目记忆和工具权限实践。
- 修订动作：补充后端调用链路、权限与数据过滤层、Prompt 元数据字段、Trace 示例、Prompt Service 分层、Claude Code 项目记忆与权限实践、生产踩坑记录、发布状态流、异常矩阵、可观测字段和审计字段。
- 结论：本章已能映射到真实后端系统，明确 Prompt 层不负责权限执行和高风险副作用操作；生产状态、异常、发布、审计和评估闭环已进入正文。

### 学习体验审稿人

- 发现问题：Prompt 概念容易碎片化，需要用同一个会议纪要助手贯穿；初学者需要看到同一输入下坏 Prompt 和好 Prompt 的输出差异；工程实现部分框架术语密度偏高；进阶实践任务缺少最小评分模板。
- 修订动作：开篇补充会议输入、坏输出和期望输出片段；在框架段落前增加“概念 -> 后端职责 -> 框架形态”映射表；在基础解释、工程实现、评估和实践任务中持续回扣会议纪要助手；为进阶实践任务增加评分字段和上线判断门槛。
